...
class RectSelector:
    def __init__(self, win, callback,rectW, rectH):
        self.win = win
        self.callback = callback
        cv.setMouseCallback(win, self.onmouse)
        self.drag_start = None
        self.drag_rect = None
        self.croped_rect = None
        self.wid = rectW
        self.hei = rectH
    ...
    def onmouse(self, event, x, y, flags, param):
        ...
        # if self.drag_start:
        #     x0, y0 = [x, y]
        #     x1, y1 = np.maximum([x0, y0], [x0 + self.wid, y0 + self.hei])
    ...
    def print(self, frame):
        if not self.croped_rect:
                return None
        x1, y1, x2, y2  = self.croped_rect
